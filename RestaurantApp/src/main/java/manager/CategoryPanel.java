...
public class CategoryPanel extends javax.swing.JPanel {
   ...
   if(n==0){
       ...
       if(menu.isEditing()==true){//тупий метод треба щось інше всунути 
            try {
               System.out.println("-----"+model.getValueAt(0, 0));
         Manager.getThread().deleteDish(28);
         System.out.println("))))))"+menu.getValueAt(0, 2).toString());
         
         Manager.getThread().addDish(Integer.parseInt(model.getValueAt(0, 0).toString()),category.getSelectedItem().toString(), model.getValueAt(0, 1).toString(), menu.getValueAt(0, 2).toString(), 111);// і цу якийсь бред лалдіводал
         }
          catch (IOException ex) {
            Logger.getLogger(CategoryPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void menuPropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_menuPropertyChange
        // TODO add your handling code here:
        /*itt++;
        System.out.println(" Щось змінюється !!!!!)"+itt);
        
            int c = menu.getEditingRow();
            int i =0;
            boolean t=true;
            while(i<massOfChangesRow.toArray().length){
            if (massOfChangesRow.get(i).equals(c)){t=false;}
            if(c<0){t=false;}
            i++;
           
        
            if(t==true){massOfChangesRow.add(menu.getEditingRow());}
        }*/
